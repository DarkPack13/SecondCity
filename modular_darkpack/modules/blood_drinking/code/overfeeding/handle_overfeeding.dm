/mob/living/carbon/human/proc/handle_overfeeding(var/mob/living/carbon/human/human_mob)
	...
	if(human_mob.stat != DEAD)
		if(isnpc(human_mob))
			...
			if(killed_count >= 5)
				SEND_SOUND(src, sound('modular_darkpack/modules/deprecated/sounds/humanity_loss.ogg', 0, 0, 50))
				to_chat(src, span_userdanger("<b>POLICE ASSAULT IN PROGRESS</b>"))
		SEND_SOUND(src, sound('modular_darkpack/modules/deprecated/sounds/feed_failed.ogg', 0, 0, 50))
