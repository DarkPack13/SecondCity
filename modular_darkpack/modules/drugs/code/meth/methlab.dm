// DARKPACK TODO - Convert this to a chem heater or chem mixer?
...
/obj/structure/methlab/movable/examine(obj/item/held_item, mob/user)
	. = ..()
	if(health == 20)
...
/obj/structure/methlab/wrench_act(mob/living/user, obj/item/tool)
...
/obj/structure/methlab/attackby(obj/item/used_item, mob/user, params)
	if(reacting)
		return FALSE
	if(istype(used_item, /obj/item/reagent_containers/applicator/pill/ephedrine))
		added_ephed += 1
		to_chat(user, span_notice("You [pick("insert", "add", "mix")] [added_ephed] [used_item] in [src]."))
		qdel(used_item)
		. = TRUE
	else if(istype(used_item, /obj/item/reagent_containers/applicator/pill/potassiodide))
		if(added_ephed != 3)
			troll_explode = TRUE
		added_iod += 1
		...
		qdel(used_item)
		. = TRUE
	else if(istype(used_item, /obj/item/reagent_containers/cup/glass/coffee/vampire))
		if(added_iod != 0)
			troll_explode = TRUE
		added_iod += 1
		...
		qdel(used_item)
		. = TRUE
	else if(istype(used_item, /obj/item/reagent_containers/cup/glass/bottle/vodka))
		if(added_iod != 1)
			troll_explode = TRUE
		added_iod += 1
		to_chat(user, span_notice("You [pick("throw", "blow", "spit")] [used_item] in [src]."))
...
#define METH_AMOUNT_PRODUCED 4
	for(var/i = 1 to METH_AMOUNT_PRODUCED)
		new /obj/item/reagent_containers/cup/glass/baggie/meth(get_turf(src))
