...
/obj/machinery/radio_tranceiver/police/proc/crime_reported(datum/source, crime, turf/location)
	...
	switch(crime)
		...
		if(CRIME_FIREFIGHT)
			radio.talk_into(radio, "An active firefight in progress has been reported at [get_area_name(location, TRUE)].", FREQ_POLICE, list(SPAN_ROBOT))
		if(CRIME_ASSAULT)
			radio.talk_into(radio, "An active assault in progress has been reported at [get_area_name(location, TRUE)].", FREQ_POLICE, list(SPAN_ROBOT))
		if(CRIME_MURDER)
			radio.talk_into(radio, "A homicide has been reported at [get_area_name(location, TRUE)].", FREQ_POLICE, list(SPAN_ROBOT))
		if(CRIME_BURGLARY)
