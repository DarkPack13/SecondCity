...
/obj/ritual_rune/abyss/pierce_the_veil/complete()
	...
	var/roll = SSroll.storyteller_roll(H.st_get_stat(STAT_INTELLIGENCE) + H.st_get_stat(STAT_OCCULT), 6, H)
	switch(roll)
		if(ROLL_SUCCESS)
			...
			H.update_sight()
		if(ROLL_FAILURE)
			to_chat(H, span_warning("The shadows slip through your fingers..."))
		if(ROLL_BOTCH)
