...
/datum/action/innate/transformation/Activate()
	...
	shapeshift_type = show_radial_menu(owner, owner, shape_names_to_image, custom_check = CALLBACK(src, PROC_REF(check_menu), owner), radius = 38)
	if(!shapeshift_type)
		return
...
/mob/living/carbon/proc/finish_transformation(shapeshift_type)
	...
	new_shape.apply_status_effect(/datum/status_effect/shapechange_mob/from_spell, caster, src)
