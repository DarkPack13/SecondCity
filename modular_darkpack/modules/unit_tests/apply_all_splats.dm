...
/datum/unit_test/apply_all_splats/Run()
	...
	npc.mind_initialize()
	test_applying_splats(npc, all_splat_types)
	...
	test_applying_splats(player, all_splat_types)

/datum/unit_test/apply_all_splats/proc/test_applying_splats(mob/living/carbon/human/human, list/splat_types)
	for (var/splat_type in splat_types)
		...
		var/datum/splat/adding_splat = human.add_splat(splat_type)
		if (!adding_splat)
			TEST_FAIL("Failed to add splat [splat_type] to mock [human.mock_client ? "player" : "NPC"].")
			continue

		var/datum/splat/gotten_splat = human.get_splat(splat_type)
		if (!gotten_splat)
			TEST_FAIL("Failed to get previously added splat [splat_type] from mock [human.mock_client ? "player" : "NPC"].")
			continue

		var/remove_success = human.remove_splat(splat_type)
		TEST_ASSERT(remove_success, "Failed to remove splat [splat_type] from mock [human.mock_client ? "player" : "NPC"].")
