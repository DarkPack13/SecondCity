...
/mob/dead/new_player/proc/check_discipline_warning()
	...
	if(discipline_points_spent > discipline_points_budget && !owner.client.prefs.read_preference(/datum/preference/toggle/is_diablerist))
...
/proc/get_discipline_point_budget(immortal_age)
	...
	if(immortal_age <= 200)
		return list(
			"points" = DISCIPLINE_BUDGET_ANCILLAE,	//Max age of 200, square root it (and round up) for points.
			"tier" = "Ancilla",
	...
	return list("points" = DISCIPLINE_BUDGET_ELDER,	//No max age, should just get access to 6+ powers based on generation and age. Leave points the same for now.
			"tier" = "Elder",
