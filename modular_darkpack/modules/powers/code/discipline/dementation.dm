...
/datum/discipline_power/dementation/passion/pre_activation_checks(mob/living/carbon/human/target)
	...
	var/mypower = SSroll.storyteller_roll(owner, applic_stats = list(STAT_CHARISMA, STAT_EMPATHY))
	switch(mypower)
...
/datum/discipline_power/dementation/the_haunting/pre_activation_checks(mob/living/carbon/human/target)
	...
	mypower = SSroll.storyteller_roll(owner, difficulty = theirpower, applic_stats = list(STAT_MANIPULATION, STAT_SUBTERFUGE), numerical = TRUE)
	if(mypower <= 0)
		to_chat(owner, span_warning("[target]'s mind is too powerful to influence!"))
		return FALSE
...
/datum/discipline_power/dementation/eyes_of_chaos/pre_activation_checks(mob/living/carbon/human/target)
	var/mypower = SSroll.storyteller_roll(owner, target, difficulty = 7, applic_stats = list(STAT_PERCEPTION, STAT_OCCULT), numerical = FALSE)
	switch(mypower)
		if(ROLL_SUCCESS)
			return TRUE
		if(ROLL_FAILURE, ROLL_BOTCH)
...
/datum/discipline_power/dementation/voice_of_madness/pre_activation_checks(mob/living/target)
	successes = SSroll.storyteller_roll(owner, difficulty = 7, applic_stats = list(STAT_MANIPULATION, STAT_EMPATHY), numerical = TRUE)
	if(successes >= 0)
...
/datum/discipline_power/dementation/total_insanity/pre_activation_checks(mob/living/carbon/human/target)
	...
	mypower = SSroll.storyteller_roll(owner, difficulty = theirpower, applic_stats = list(STAT_MANIPULATION, STAT_INTIMIDATION), numerical = TRUE)
	if(mypower <= 0)
