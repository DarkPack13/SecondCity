/obj/item/smartphone
	...
	//password and lockscreen call checking
...
/obj/item/smartphone/proc/check_password(mob/living/user, autocheck)
	...
	if(tgui_input_text(user, "Enter the pin number for [src]:", "Pin Input", max_length=4, multiline=FALSE) != phone_password)
		to_chat(user, span_alert("The pin you entered for [src] is incorrect."))
		return FALSE
	else
		to_chat(user, span_notice("You correctly enter the pin for [src]."))
		return TRUE
