...
/datum/preference_middleware/stats/get_ui_data(mob/user)
	...
	for(var/typepath in preferences.preference_storyteller_stats)
		...
		stat_data["score"] = stat.get_score(include_bonus = FALSE, include_stat_clamps = FALSE)
...
/datum/preference_middleware/stats/proc/increase_stat(list/params, mob/user)
	...
	var/old_value = stat_path.get_score(include_bonus = FALSE, include_stat_clamps = FALSE)


	if(!stat_path.can_increase_score(1)) // Have we reached the max_score of the stat?
		return FALSE // If we have, then return early.

	if((stat_path.get_score(include_bonus = FALSE, include_stat_clamps = FALSE) + 1) > stat_path.starting_score)
	...
	var/new_value = stat_path.get_score(include_bonus = FALSE, include_stat_clamps = FALSE)
