...
/datum/controller/subsystem/stats/proc/sanitize_stat_list(passed_stats)
	var/new_list = list()
	for(var/stat_path in GLOB.storyteller_stats)
		var/datum/st_stat/stat = GLOB.storyteller_stats[stat_path]
...
/datum/controller/subsystem/stats/proc/sanitize_points_list(passed_stats)
	if(passed_stats)
		return passed_stats
	var/new_list = init_storyteller_stats()
	return new_list
