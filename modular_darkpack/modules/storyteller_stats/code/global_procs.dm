...
/proc/update_middleware_stats(list/preference_storyteller_stats)
	...
	stat_permenant_willpower.add_stat_mod(clamp(-(stat_permenant_willpower.get_score(include_bonus = FALSE, include_stat_clamps = FALSE) - 10), 0, stat_courage.get_score(include_bonus = TRUE)), "COURAGE")
