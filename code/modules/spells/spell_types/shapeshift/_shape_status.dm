...
/datum/status_effect/shapechange_mob/from_spell/restore_caster(kill_caster_after)
	...
	if(owner?.contents)
		...
		for(var/atom/movable/thing as anything in owner.contents)
			if(thing == caster_mob || HAS_TRAIT(thing, TRAIT_NOT_BARFABLE) || istype(thing, /obj/item/bodypart)) //TFN EDIT, ORIGINAL: if(thing == caster_mob || HAS_TRAIT(thing, TRAIT_NOT_BARFABLE))
				continue
