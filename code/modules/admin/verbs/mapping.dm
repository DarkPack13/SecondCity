...
ADMIN_VERB(create_mapping_job_icons, R_DEBUG, "Generate job landmarks icons", "Generates job starting location landmarks.", ADMIN_CATEGORY_MAPPING)
	...
	for(var/job_type in valid_subtypesof(/datum/job/vampire)) // DARKPACK EDIT CHANGE
	...
	fcopy(final, "modular_darkpack/modules/jobs/icons/landmarks.dmi") // DARKPACK EDIT CHANGE
